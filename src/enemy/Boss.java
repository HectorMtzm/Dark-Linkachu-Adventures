	...
	public void draw(Graphics g) {
		...
		if (GameFrame.getNumOfHearts() > 0) {
			for (int i = 0, heartDistance = 30; i < GameFrame.getNumOfHearts(); i++, heartDistance += 30)
				g.fillRect((int) this.x + heartDistance + 10, (int) this.y - 30, 30, 8);
		}
	}
	...
	public void move() {
		...
		for (ProjectileObjects projectile : GameFrame.getAllProjectiles()) {
			if (projectile instanceof Arrow) {
				if (this.getBoundsTOP().intersects(projectile.getBounds()) || this.getBoundsBottom().intersects(projectile.getBounds())) {
